...
const Contact: React.FC = () => {
  ...
  const companyEmail = 'sales@discoverx.com.cn';

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // In a real application, this would be an API call to a backend service.
    // For this prototype, we simulate the "automatic send" logic.
    console.log(`Sending message to ${companyEmail}...`);
    setSubmitted(true);
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <div className="grid md:grid-cols-2 gap-16">
        <div>
          ...
          <div className="space-y-6">
            <div className="flex items-start">
              ...
              <div>
                ...
                <p className="text-slate-600 text-sm">上海市浦东新区张江高科技园区达尔文路 88 号</p>
              </div>
            </div>
          ...
          <div className="mt-12 rounded-2xl overflow-hidden h-64 bg-slate-200 shadow-inner border border-slate-200">
            <img src="https://picsum.photos/seed/map/800/400" className="w-full h-full object-cover grayscale opacity-60" alt="Map placeholder" />
          </div>
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <select className="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#4B827E]/50 focus:bg-white transition-all">
                  <option>1, 产品咨询</option>
                  <option>2, 定制开发</option>
                  <option>3, 技术支持</option>
                  <option>4, 索取资料</option>
                </select>
              </div>
              <div>
